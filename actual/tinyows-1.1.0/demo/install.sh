...
PGSHARE=/usr/share/postgresql/9.4/contrib/postgis-1.5
PGUSER=postgres
SHP2PGSQL=/usr/bin/shp2pgsql
DB=tinyows_demo

if [ -d /usr/share/postgresql/9.4/contrib/postgis-2.0 ]; then
	PGSHARE=/usr/share/postgresql/9.4/contrib/postgis-2.0
elif [ -d /usr/share/postgresql/9.4/contrib/postgis-1.5 ]; then
	PGSHARE=/usr/share/postgresql/9.4/contrib/postgis-1.5
else
	echo "Unable to find PostGIS dir in /usr/share/postgresql/9.4/contrib/" && exit 1
fi
